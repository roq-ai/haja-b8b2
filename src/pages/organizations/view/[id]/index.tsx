...
import { deleteBlogPostById } from 'apiSdk/blog-posts';
  ...
  const { data, error, isLoading, mutate } = useSWR<OrganizationInterface>(
    ...
    () =>
      getOrganizationById(id, {
        relations: ['user', 'blog_post', 'course'],
      }),
  );

  const blog_postHandleDelete = async (id: string) => {
    setDeleteError(null);
    try {
        ...
        {isLoading ? (
          ...
          <>
            ...
            <Box>
              ...
              {hasAccess('blog_post', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    <Text fontSize="lg" fontWeight="bold">
                      Blog Posts
                    </Text>
                    <NextLink passHref href={`/blog-posts/create?organization_id=${data?.id}`}>
                  ...
                  <TableContainer mb={4}>
                    <Table variant="simple">
                      ...
                      <Tbody>
                        {data?.blog_post?.map((record) => (
                          <Tr
                            cursor="pointer"
                            onClick={() => router.push(`/blog-posts/view/${record.id}`)}
                            key={record.id}
                          >
                            <Td>{record.title}</Td>
                            ...
                            <Td>
                              {hasAccess('blog_post', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                                <NextLink
                                  onClick={(e) => e.stopPropagation()}
                                  passHref
                                  href={`/blog-posts/edit/${record.id}`}
                                >
                              ...
                              {hasAccess('blog_post', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                                <IconButton
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    blog_postHandleDelete(record.id);
                                  }}
                                  colorScheme="red"
                                  variant="outline"
                                  aria-label="edit"
              ...
              {hasAccess('course', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/courses/create?organization_id=${data?.id}`}>
                      <Button colorScheme="blue" mr="4" as="a">
                        Create
                      </Button>
                    </NextLink>
                    ...
                    <Table variant="simple">
                      ...
                      <Tbody>
                        {data?.course?.map((record) => (
                          <Tr
                            cursor="pointer"
                            onClick={() => router.push(`/courses/view/${record.id}`)}
                            key={record.id}
                          >
                            <Td>{record.name}</Td>
